/**
 * 샘플 DB 생성 스크립트 — 앱 수동 실행 시 예제 데이터로 사용.
 * 실행: node example/create-sample-db.js
 * 결과: example/sample.db 생성
 */
...
db.prepare(`INSERT INTO operations (task_id,operation_type,agent_platform,summary,
  started_at,completed_at,tool_name,retry_count,input_tokens,output_tokens,duration_seconds)
  VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
  .run('FIX-T001','start','claude_code',null,'2026-03-15 10:00:00',null,null,0,null,null,null)

db.prepare(`INSERT INTO operations (task_id,operation_type,agent_platform,summary,
  tool_name,skill_name,input_tokens,output_tokens,duration_seconds)
  VALUES (?,?,?,?,?,?,?,?,?)`)
  .run('FIX-T001','progress','claude_code','DB 스키마 작성 완료','Write','tdd',800,200,30)

db.prepare(`INSERT INTO operations (task_id,operation_type,agent_platform,summary,
  tool_name,mcp_name,retry_count,input_tokens,output_tokens,duration_seconds)
  VALUES (?,?,?,?,?,?,?,?,?,?)`)
  .run('FIX-T001','progress','claude_code','엔드포인트 2/3 완료','Edit','context7',1,1200,450,65)

db.prepare(`INSERT INTO operations (task_id,operation_type,agent_platform,summary,
  completed_at,tool_name,input_tokens,output_tokens,duration_seconds)
  VALUES (?,?,?,?,?,?,?,?,?)`)
  .run('FIX-T001','complete','claude_code','로그인 API 구현 완료','2026-03-15 11:00:00','Bash',400,120,15)

// Operations for T002
db.prepare(`INSERT INTO operations (task_id,operation_type,agent_platform,summary,
  started_at,tool_name,retry_count,input_tokens,output_tokens,duration_seconds)
  VALUES (?,?,?,?,?,?,?,?,?,?)`)
  .run('FIX-T002','start','claude_code',null,'2026-03-15 11:05:00',null,0,null,null,null)

db.prepare(`INSERT INTO operations (task_id,operation_type,agent_platform,summary,
  tool_name,skill_name,input_tokens,output_tokens,duration_seconds)
  VALUES (?,?,?,?,?,?,?,?,?)`)
  .run('FIX-T002','progress','claude_code','입력값 검증 로직 작성 중','Edit','tdd',950,380,48)
