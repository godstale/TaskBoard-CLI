...
| `R` | 데이터 새로고침 |
| `Q` | 종료 |
| `↑` / `↓` | Workflow 선택 (Workflows); Task 선택 (Dashboard); 작업 이력 스크롤 (Task Ops) |
| `Enter` | Workflow 선택 (Workflows); 선택한 Task의 Task Ops 화면으로 이동 (Dashboard) |
| `←` / `→` | Task 선택 (Task Ops 화면) |
...
다른 화면들은 Workflows 화면에서 **Workflow**를 선택하기 전까지 잠금(🔒) 상태로 표시됩니다.

| 화면 | 설명 |
|------|------|
| **Workflows** | 프로젝트 내의 워크플로우를 선택하고 진행 상태를 확인 |
| **Dashboard** | Epic/Task 계층 + 진행률 바 |
| **Task Ops** | 텍스트 작업 이력 타임라인 및 태스크별 상세 이력 |
| **Resources** | 리소스 파일 목록 + 타입 컬러 |
| **Settings** | Key/Value 설정 테이블 + 프로젝트 체크포인트 |
